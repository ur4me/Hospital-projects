...
prediction <- predictions(predict(rf_model, test))
prediction <- ifelse(prediction < 0.5 , 0 ,1)
confusionMatrix(as.factor(prediction), as.factor(test$Rehab))[["overall"]][["Accuracy"]] #I got 0.6810934
...
for(i in 1:4){
  ...
  for(n in 80:100) {
    ...
    result <- data.frame(percentage = n, accuracy = confusionMatrix(as.factor(prediction), as.factor(testing$Rehab))[["overall"]][["Accuracy"]])
    outlier.checking <- rbind(outlier.checking,result)
  }
  
  comparison[c] <- outlier.checking$accuracy
  c <- c + 1
...
comparison$average <- (comparison$V2 + comparison$V3 + comparison$V4 + comparison$V5)/4
comparison <- comparison %>% arrange(desc(average))
max(comparison$average) #0.7660998
...
prod.params <- prod.params
set.seed(1234)
rehab_predictor <- ranger(formula = Rehab~.-FiscalYear,
                   data=train,
                   num.trees = 50,
                   mtry=prod.params$mtry,importance = 'impurity',
                   min.node.size = prod.params$nodesize)
...
#evaluation
prediction <- predictions(predict(rehab_predictor, test))
prediction <- ifelse(prediction < 0.5 , 0 ,1)
confusionMatrix(as.factor(prediction), as.factor(test$Rehab))[["overall"]][["Accuracy"]] #I got 0.7054632

#save Rehab predictor
save(rehab_predictor, file = "D:/Stroke/Model/rehab_predictor.rda")
...
dtrain <- xgb.DMatrix(as.matrix(withoutRV),label = train1$Rehab)
...
#evaluation
confusionMatrix(as.factor(test1$Rehab), as.factor(prediction_xgb))[["overall"]][["Accuracy"]] #0.6924829
...
confusionMatrix(as.factor(prediction_lm), as.factor(test1$Rehab))[["overall"]][["Accuracy"]] #0.6355353
